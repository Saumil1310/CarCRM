...
def inventory(request):
	...
	if request.POST:
		...
		InventoryMaster.objects.create(CarName=request.POST.get('cname'),OldCust=request.POST.get('old_cus'),CarModel=request.POST.get('cmodel'),Mfg=request.POST.get('mfg'),CarImage=request.FILES['car_image'],RegNo=request.POST.get('reg_no'),Engine=request.POST['engine'],Milage=request.POST.get('milage'),BuyPrice=request.POST['buy_price'],SellPrice=request.POST['sell_price'],Date=request.POST['date'], BodyShop_Amount=request.POST.get('bodyshop'),BodyShop_Details=request.POST.get('bodyshop1'),Mechanic_Amount=request.POST.get('mechanic'),Mechanic_Details=request.POST.get('mechanic1'),MOT_Amount=request.POST.get('mot'),MOT_Details=request.POST.get('mot1'),Travelling_Amount=request.POST.get('travelling'),Travelling_Details=request.POST.get('travelling1'),Fuel_Amount=request.POST.get('fuel'),Fuel_Details=request.POST.get('fuel1'),IsActive=1)
		...
		for img,des in zip(Docs,texts):
			images=CarDocMaster.objects.create(CarID=car_id_instance,Doc=img,DocDescription=des,CarName=request.POST['cname'],RegNo=request.POST['reg_no'])
		############################
		total = []
		...
		LeadgerMaster.objects.create(Date=request.POST['date'],CarName=request.POST.get('cname'),RegNo=request.POST.get('reg_no'), KWNo=kw_no,FinalPrice=Purchase_price, Vtype='Buy')
...
def update_car(request,id):
	...
	if request.POST:
		InventoryMaster.objects.filter(RegNo=car_regno).update(CarName=request.POST['cname'],OldCust=request.POST['old_cus'],CarModel=request.POST['cmodel'],Mfg=request.POST.get('mfg'),RegNo=request.POST['reg_no'],Engine=request.POST['engine'],Milage=request.POST['milage'],BuyPrice=request.POST['buy_price'],SellPrice=request.POST['sell_price'],Date=request.POST['date'],BodyShop_Amount=request.POST.get('bodyshop'),BodyShop_Details=request.POST.get('bodyshop1'),Mechanic_Amount=request.POST.get('mechanic'),Mechanic_Details=request.POST.get('mechanic1'),MOT_Amount=request.POST.get('mot'),MOT_Details=request.POST.get('mot1'),Travelling_Amount=request.POST.get('travelling'),Travelling_Details=request.POST.get('travelling1'),Fuel_Amount=request.POST.get('fuel'),Fuel_Details=request.POST.get('fuel1'))
		...
		texts=request.POST.getlist('doc[]')
		for img,des in zip(Docs,texts):
			images=CarDocMaster.objects.filter(CarID=id).update(CarID=car_id_instance,Doc=img,DocDescription=des,RegNo=request.POST['reg_no'],CarName=request.POST['cname'])
		########################
		total = []
		total.append(int(request.POST['bodyshop']))
		total.append(int(request.POST['mechanic']))
...
def add_lead(request):
	...
	if request.POST:
		...
		LeadMaster.objects.create(LeadName=request.POST.get('name'),LeadEmail=request.POST.get('email'),LeadAddress=request.POST.get('address'),LeadMobile=request.POST.get('phone'),Choices=my_cars_string,Payment=request.POST['optradio'],OfferPrice=request.POST.get('sell-price'),Comment=request.POST.get('comment'),Date=request.POST['date'],IsActive=1)
...
def lead_update(request,id):
	...
	if request.POST:
		my_cars_list = request.POST.getlist('selected') ##To get selected car list 
		my_cars_string = ','.join(my_cars_list)
		LeadMaster.objects.filter(LeadID=id).update(LeadName=request.POST.get('name'),LeadEmail=request.POST.get('email'),LeadAddress=request.POST.get('address'),LeadMobile=request.POST.get('phone'),Choices=my_cars_string,Payment=request.POST['optradio'],OfferPrice=request.POST.get('sell-price'),Comment=request.POST.get('comment'))
...
def lead_convert(request):
	if request.POST:
		...
		CustomerMaster.objects.create(CustName=lead_data['LeadName'],CustEmail=lead_data['LeadEmail'],CustMobile=lead_data['LeadMobile'],PurchasedCar=request.POST['car_name'],Amount=request.POST['amount'],OfferPrice=lead_data['OfferPrice'],Payment=lead_data['Payment'],Comment=lead_data['Comment'],Date=request.POST['date'],IsActive=1)
		...
		SoldCarsMaster.objects.create(CarName=car_data['CarName'],OldCust=car_data['OldCust'],CarModel=car_data['CarModel'],Mfg=car_data['Mfg'],CarImage=car_data['CarImage'],RegNo=car_data['RegNo'],Engine=car_data['Engine'],Milage=car_data['Milage'],KWNo=car_data['KWNo'],BuyPrice=car_data['BuyPrice'],SellPrice=car_data['SellPrice'],CurrentCust=lead_data['LeadName'],Amount=request.POST['amount'],Total_Expenses=total_exp['Total_Expenses'],Buy_EXP=buy_exp,Profit=profit,Date=request.POST['date'])
		LeadgerMaster.objects.filter(RegNo=request.POST['RegNo']).update(Date=request.POST['date'],CarName=car_data['CarName'],RegNo=car_data['RegNo'], KWNo=car_data['KWNo'],Amount=request.POST['amount'],Vtype='Sales',FinalPrice='0')
...
def admin_register(request):
	...
	if request.POST:
		if not AdminMaster.objects.filter(AdminEmail=request.POST['Email']).exists():
			AdminMaster.objects.create(AdminEmail=request.POST['Email'],AdminPassword=request.POST['Password'],AdminUsername=request.POST['Username'])
...
def admin_login(request):
	context={}
	if request.POST:
		if AdminMaster.objects.filter(AdminEmail=request.POST['Email'],AdminPassword=request.POST['Password']).exists():
			return redirect('car_list')
		else:
...
def expense(request):
	...
	if request.POST:
		request.session['from']=request.POST['f_date']
		request.session['to']=request.POST['t_date']
		expense_details=InventoryMaster.objects.filter(Date__range=[request.POST['f_date'],request.POST['t_date']]).values()
...
def profit(request):
	...
	if request.POST:
		request.session['from']=request.POST['f_date']
		request.session['to']=request.POST['t_date']
		sold_details=SoldCarsMaster.objects.filter(Date__range=[request.POST['f_date'],request.POST['t_date']]).values()
